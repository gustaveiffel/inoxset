...
## [0.1.0-alpha.5] — 2026-07-08

### Added

- **Interval membership (range writes).** New API family for interval-shaped facts — stays, subscriptions, coverage periods, contracts: `put_bitmap_range`, `put_ids_range`, `remove_bits_range`, `remove_ids_range` (+ `put_uuids_range`/`remove_uuids_range` behind the `uuid` feature). One call writes/tombstones every period bucket from `first` to `last` (inclusive) **atomically with respect to readers** — a partially applied interval is structurally impossible. Rollup ancestors are applied once per distinct ancestor instead of once per bucket, the dictionary is resolved once per call, and buckets share one bitmap allocation. Invalid bounds are errors (fail-loud writes), ranges are capped at 366×24 buckets.
...
- **`IndexFreshness::Immediate` is rejected at open** with a configuration error instead of silently degrading to a disabled index (it was never implemented; callers believed they had the freshest index while every lookup scanned bitmaps).
